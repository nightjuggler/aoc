...
def main(input_file):
	...
	turn_on = lambda row, x1, x2: [row[x]+1 for x in range(x1, x2+1)]
	turn_off = lambda row, x1, x2: [max(row[x]-1, 0) for x in range(x1, x2+1)]
	toggle = lambda row, x1, x2: [row[x]+2 for x in range(x1, x2+1)]
